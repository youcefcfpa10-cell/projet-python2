...
def open_PV2_2_form():
    ...
    # ---------------- العنوان ----------------
    tk.Label(root, text="إنجاز المهام",
             bg=form_bg, fg=label_fg,
             font=("Arial", 22, "bold italic")).pack(pady=10)

    tk.Label(root,
             text="مدى تحقيق إنجاز مهام الرقابة و الإنضباط",
             bg=form_bg, fg=label_fg,
    ...
    elements = [
        ...
        "تسيير المكتبة",
        "الوقاية و الأمن",
        "تحضير أشغال المجلس التأديبي",
        "المجموع"
    ]

    base_marks = [4, 4, 4, 4, 4]
    ...
    def save_data():
        data = {
            "title": "مدى تحقيق إنجاز مهام الرقابة و الإنضباط",
            "marks": [v.get() for v in text_vars],
            "base_total": base_total_var.get(),
            "total": total_var.get()
        }
        save_to_json_shared("PV2_2", data, "data1.json")
